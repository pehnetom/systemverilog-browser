...
export default class SystemVerilogSearch {
    ...
    searchProject(){
      ...
      packageTree = [];
      this.folderTree.forEach(function(item){
        item.forEach(function(subItem){
          if(subItem.type.id == subItem.type.hashCode("package")){
            packageTree.push(subItem);
          }
        });
      });
        ...
        if (nextType.border == 0){
          ...
          position = retVal.position;
            ...
            if (nextType.withRange == 0) {
              currentParent.elements.push(new SystemVerilogCodeElem(foundElementsStack[position][0],foundElementsStack[position][0],foundElementsStack[position][1],svFileInfo));
              position += 1;
            } else {
    ...
    searchForFileElements(file){
      ...
      svFileInfo = file.getPath().match(/(?!\/)\w*(\..*)$/);
      ...
      file.scanInRange(/\b((package)\s(.*;|\s;|;)|(endpackage.*)$)/g, file.getRange(), event => foundElementsStack.push([event.range,event.matchText]))
      ...
      foundElementsStack.unshift([new Range([[0,0],[0,0]]),"svFile "+svFileInfo]);
      retVal = this.searchElement(foundElementsStack,0,svFileInfo);
