...
export default class SystemVerilogSearch {
      ...
      while(Object.keys(foundElementsStack).length > position){
        ...
        if (nextType.border == 0){
          ...
          position = retVal.position;
            ...
            if (nextType.withRange == 0 || nextType.extern == 1) {
    ...
    searchForFileElements(file){
      ...
      file.scanInRange(/\b((class\s){1}(\w*)(\sextends.*;|\s;|;)|(endclass.*)$)/g, file.getRange(), event => foundElementsStack.push([event.range,event.matchText]))
      file.scanInRange(/\b((virtual\s)*(extern\s)*(virtual\s)*(function\s){1}(.*;|\s;|;)|(endfunction.*)$)/g, file.getRange(), event => foundElementsStack.push([event.range,event.matchText]))
      file.scanInRange(/\b((task\s){1}(.*;|\s;|;)|(endtask.*)$)/g, file.getRange(), event => foundElementsStack.push([event.range,event.matchText]))
