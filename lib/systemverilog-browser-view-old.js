...
export default class SystemverilogBrowserView {
  ...
  constructor(serializedState) {
    ...
    updatePanel(message,item){
      ...
      searchResult = searchObj.searchCurrentEditor();
    }
    ...
    displayElements(message,item,self){
    ...
    getSVClasses(message,item,self){
      ...
      for (var i = 0; i < Object.keys(classStack).length; i++) {
        (function () {
          ...
          classElem.innerHTML = `<span>&emsp;<span class="fas fa-plus-circle"></span>&emsp;<b>${classStack[funcId][1]}</b></span>`;
          // classElem.innerHTML = `<span>&emsp;<span class="fas fa-minus-circle"></span>&emsp;<b>${classStack[funcId][1]}</b></span>`;
    ...
    funcClick(message,item,self,inputRange){
      ...
      decoration = item.decorateMarker(marker, {type:'highlight',class:'highlighStyle2'});
