...
export default class SystemverilogBrowserView {
  ...
  constructor(serializedState) {
    ...
    getSVClasses(message,item,self){
      ...
      var classStack = [];
      var endClassStack = [];
      item.scanInBufferRange(/\b(class)\s(\w*)/g, item.buffer.getRange(), event => classStack.push([event.range,event.matchText]))
    ...
    funcClick(message,item,self,inputRange){
      ...
      decoration = item.decorateMarker(marker, {type: ['line','highlight'],class: 'find-result'});
    }
